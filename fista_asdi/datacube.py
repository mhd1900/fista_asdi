...
class DataCubeASDI:
    def __init__(self, nproc=1, folder=None, verbose=False, imlib='vip-fft', interpol="lanczos4",
        ...
        @params:
            <int> nproc: 
                Number of processor to use, namely when computing the SNR map.
            <string> folder: 
            ...
            <string> cube, angles, wavelengths, psf, pxscale:
                path to the data cube, the parallactic angles, wavelengths, psf, and pixel scale fits files respectively.
    ...
    def find_scale_vector(self, **kwargs):
        if self.is_preproc:
    ...
    def cancel_preproc(self, forced_free_memory=False):
        ...
        @params: 
            <bool> forced_free_memory: If the memory of the preprocessed cube is not freed, you can try to set this parameter to True.
        """
    ...
    def inject_companions(self, planet, n_branches=1):
        ...
        @params:
            <dict> planet:
                Dictionnary containing 4 keys: the 'name' of the planet is a string, the 'flux' is a 1D vector, 
                the 'dist' is a float and the 'angle' is a float too.
        """
    ...
    def planet_free(self, planet_id):
        ...
        @params:
            <string> planet_id: name of the planet.
        """
        if self.is_preproc: 
    ...
    def apply_rotation(self, cube, inplace=False, inverse=False):
        """
        ROtate all the frames of a ASDI datacube according to the parallactic angle given in the 1D array 'self.angles'
        @pre:
    ...
    def apply_downscaling(self, cube, t=None, inverse=False, imlib=None):
        """
        Rescale all the frames of a ASDI datacube according to the scale factor given in the 1D array 'self.scale_vec'
        @pre:
            <4D array> datacube: datacube to rescale
            <1D array> sc_factor: scaling factor to apply for each wavelength
            <bool> inverse: if True, apply the inverse transform
